...
const Enquire = () => {
    ...
    useEffect(() => {
        const fetchMaintenance = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/type/api/maintenance/`);
    ...
    <h1 className="text-3xl font-bold">MAINTENANCE REQUESTS</h1></div>
    <div className={`flex-grow p-4 mr-60 transition-all duration-300 ease-in-out ${isSidebarReduced ? 'ml-20' : 'ml-64'}`}>
              <table className="min-w-full border border-gray-200 mt-40 ml-40">
                ...
                <tbody>
                  {maintenance.map((tenant) => (
                    <tr key={tenant.id} className="border-b">
                      <td className="pl-20">{tenant.name}</td>
                      <td className="pl-20">{tenant.contact}</td>
                     ...
                     {tenant.date}
                      ...
                      <td className="pl-20">{tenant.description}</td>                 
                    </tr>
                  ))}
                </tbody>
              </table>
