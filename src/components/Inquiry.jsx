...
import { AiOutlineMenu, AiOutlineArrowLeft, AiOutlineHome } from 'react-icons/ai'; 
...
const Enquire = () => {
    ...
    useEffect(() => {
        const fetchTenants = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/type/api/price/`);
    ...
    <h1 className="text-3xl font-bold">INQUIRIES</h1></div>
    <div className={`flex-grow p-4 mr-60 transition-all duration-300 ease-in-out ${isSidebarReduced ? 'ml-20' : 'ml-64'}`}>
              <table className="min-w-full border border-gray-200 mt-40 ml-40 ">
                <thead>
                ...
                <tbody>
                  {tenants.map((tenant) => (
                    <tr key={tenant.id} className="">
                      <td className="pl-10">{tenant.fname}</td>
                      <td className="pl-10">{tenant.lname}</td>
                      <td className="pl-20 hover:text-blue-700 ">
                        <a href={`mailto:${tenant.email}`}>{tenant.email}</a>
                      </td>
                      <td className="pl-10 hover:text-blue-700 ">
                        <a href={`tel:${tenant.contact}`}>{tenant.contact}</a>
                      </td>
                      <td className="pl-10">{tenant.company}</td>
                      <td className="pl-10">{tenant.units}</td>
